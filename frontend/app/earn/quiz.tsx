...
export default function QuizScreen() {
  ...
  const onPick = (j: number) => {
    setAnswers((a) => ({ ...a, [step]: j }));
  };
    ...
    Alert.alert('Watch ad', 'Watch a short ad to claim your reward', [
      { text: 'Cancel', style: 'cancel', onPress: () => setSubmitting(false) },
      {
        ...
        onPress: async () => {
          ...
          if (!earned) {
            Alert.alert('Ad incomplete', 'You must finish watching the ad. Start the quiz again to claim your reward.');
            // Reset task — user must restart quiz.
            setAnswers({});
            setStep(0);
            setSubmitting(false);
            return;
          ...
          try {
            const r = await api('/quiz/submit', { method: 'POST', body: JSON.stringify({ answers, ad_watched: true }) });
  ...
  return (
    <SafeAreaView style={styles.c} edges={['top']}>
      <View style={styles.head}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.headTitle}>Daily Quiz</Text>
      ...
      {loading ? (
        ...
        <View style={{ flex: 1 }}>
          ...
          <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 32 }} showsVerticalScrollIndicator={false}>
            {current && (
              <View style={styles.qBox} testID={`quiz-q-${step}`}>
